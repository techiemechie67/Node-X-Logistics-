...
if AsyncOpenAI is not None and DEEPSEEK_API_KEY and DEEPSEEK_API_KEY != "your_key_here":
    client = AsyncOpenAI(
...
async def generate_decision_explanation(asset_state: Any, deterministic_decision: Any) -> str:
    ...
    # 3. Check client readiness and execute DeepSeek completion
    global client
    current_key = os.getenv("DEEPSEEK_API_KEY", "").strip()
    if client is None and AsyncOpenAI is not None and current_key and current_key != "your_key_here":
        client = AsyncOpenAI(
            api_key=current_key,
...
async def underwrite_supply_chain_agent(
    ...
    """
    Executes the Autonomous Supply-Chain Underwriter AI agent using DeepSeek JSON Mode.
    ...
    effective_key = (api_key or os.getenv("DEEPSEEK_API_KEY", "")).strip()
    base_url = os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com").strip()

    last_api_error = None
    # 1. Attempt live DeepSeek AI Chat Completion with strict JSON Mode
    if effective_key and effective_key != "your_key_here" and AsyncOpenAI is not None:
        ...
        try:
            live_client = AsyncOpenAI(api_key=effective_key, base_url=base_url)
            ...
            if "underwriting_decision" in parsed:
                parsed["engine_source"] = "DEEPSEEK_LIVE"
                return parsed
        except Exception as e:
            last_api_error = str(e)
            print(f"[DeepSeek Live Underwriter Notice] Live API failed ({e}), falling back to deterministic underwriter.")
